...
export TEAMS_MSG='{ "@type": "MessageCard", "@context": "http://schema.org/extensions", "themeColor": "'$TEAMS_COLOR'", "summary": "'$TEAMS_TITLE' - '$TEAMS_SUBTITLE'", "sections": [{ "activityTitle": "'$TEAMS_TITLE'", "activitySubtitle": "'$TEAMS_SUBTITLE'", "activityImage": "'$TEAMS_IMAGE'", "markdown": true, "facts": [ { "name": "Message", "value": "'$*'" }] }] }'

curl --request POST --header "Content-type: application/json; charset=utf-8" \
--data "$TEAMS_MSG" "$TEAMS_WEBHOOK"
